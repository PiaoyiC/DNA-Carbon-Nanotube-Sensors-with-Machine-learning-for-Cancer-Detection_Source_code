...
class AdvancedFeatureInteractions:
    ...
    def __init__(self,
        ...
        # Record whether fitting has been completed
    ...
    def fit_transform(self, X, y=None):
        ...
        try:
            ...
            # 3. Post-interaction standardization
    ...
    def _gaussian_interaction(self, X):
        ...
        for i in range(n_features):
            # Calculate local density estimate
    ...
    def _select_features(self, X, y):
        ...
        if self.selection_method == 'mutual_info':
            ...
            try:
                ...
                # Select features that Boruta considers important
...
class RandomForestModel:
    ...
    def load_data(self):
        ...
        try:
            ...
            features = df.iloc[:, 1:14].values  # Columns 3-14 are features
    ...
    def apply_adasyn(self, X, y, positive_class):
        ...
        try:
            ...
            sample_indices = np.concatenate([
                ...
                np.full(len(X_resampled) - len(X), -1)  # -1 indicates synthetic data
            ])

            return X_resampled, y_binary_resampled, sample_indices

        except ValueError as e:
            # Handle possible ADASYN errors, such as too few minority class samples
    ...
    def process_fold(self, X, y_binary, sample_indices, train_index, test_index, positive_class, fold_idx):
        ...
        # Use original data for test evaluation
        ...
        # If test set contains only one class, log a warning
        ...
        thresholds = np.arange(0.1, 0.7, 0.025)  # More granular threshold range
        best_f1 = 0
        ...
        test_precision = precision_score(y_test_original, y_pred, average='weighted', zero_division=0)
    ...
    def _optimize_hyperparameters(self, X_train, y_train):
        ...
        neg_count = sum(y_train == 0)
        pos_count = sum(y_train == 1)
        base_class_ratio = neg_count / max(1, pos_count)  # Avoid division by zero

        def objective(n_estimators, max_depth, min_samples_split, min_samples_leaf,
                      max_features_ratio, class_weight_multiplier):
            """Bayesian optimization objective function - Random Forest"""
            # Transform parameters
            n_estimators = int(n_estimators)
            max_depth = int(max_depth)
            min_samples_split = int(min_samples_split)
            min_samples_leaf = int(min_samples_leaf)
            max_features = max(0.1, min(0.999, max_features_ratio))  # Convert to ratio
            ...
            for train_idx, val_idx in cv.split(X_train, y_train):
                ...
                precision = precision_score(y_val_cv, best_pred, average='weighted', zero_division=0)
            ...
            param_str = "n_est={}, depth={}, min_split={}, min_leaf={}, max_feat={:.2f}".format(
        ...
        # Use parameter search space optimized for random forest
        param_ranges = {
            ...
            'class_weight_multiplier': (0.0, 5.0)
        }
        ...
        best_params['max_features'] = max_features_ratio  # Keep as ratio
        best_params.pop('max_features_ratio', None)  # Remove original ratio parameter
    ...
    def _create_model(self, params):
        ...
        params_with_defaults = {
            ...
            'max_depth': params.get('max_depth', None),  # RandomForest can accept None
            'min_samples_split': int(params.get('min_samples_split', 2)),
            'min_samples_leaf': int(params.get('min_samples_leaf', 1)),
            'max_features': params.get('max_features', 'sqrt'),  # Use ratio or 'sqrt'/'auto'
    ...
    def plot_roc_curve(self, results, positive_class):
        ...
        for idx, result in enumerate(results):
            ...
            # Set ROC curve thickness for each fold to 2
        ...
        # Set mean ROC curve thickness to 5
    ...
    def save_results(self, all_results):
        ...
        with pd.ExcelWriter(output_path) as writer:
            ...
            for class_name, results in all_results.items():
                ...
                # Try to get class weight information
                class_weights = []
                for r in results:
                    cw = r['best_params'].get('class_weight', None)
                    if isinstance(cw, dict) and 1 in cw:
                        class_weights.append(cw[1])
                    else:
                        class_weights.append(None)
                # Calculate average
                valid_weights = [w for w in class_weights if w is not None]
                avg_class_weight = np.mean(valid_weights) if valid_weights else None
                ...
                summary_data.append({
                    ...
                    'Mean Class Weight': avg_class_weight,
            ...
            for class_name, results in all_results.items():
                ...
                fold_data = []
                for r in results:
                    # Get class weight information
                    class_weight = r['best_params'].get('class_weight', None)
                    class_weight_positive = class_weight[1] if isinstance(class_weight,
                                                                          dict) and 1 in class_weight else None
                    ...
                    fold_data.append({
                        ...
                        'ROC AUC': r['roc_auc'],
                        'PR AUC': r['pr_auc'],
                        'False Negative Rate': r['fnr'],  # False negative rate
                        'Optimal Threshold': r['optimal_threshold'],  # Optimal threshold
                        'n_estimators': int(r['best_params'].get('n_estimators', 0)),
                        'max_depth': r['best_params'].get('max_depth', None),
                        'min_samples_split': int(r['best_params'].get('min_samples_split', 2)),
                        'min_samples_leaf': int(r['best_params'].get('min_samples_leaf', 1)),
                        'max_features': r['best_params'].get('max_features', 'auto'),
...
def main():
    ...
    config = {
        ...
        'output_path': os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'Results', 'Multicancer detection', '02_Algorithm optimization', 'RF'),  # Directory path for saving output results
        'output_filename': 'RandomForest_results.xlsx',
