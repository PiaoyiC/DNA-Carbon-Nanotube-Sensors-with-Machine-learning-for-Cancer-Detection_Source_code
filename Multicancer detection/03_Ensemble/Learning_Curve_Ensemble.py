# Learning Curve version of Ensemble_RF+XGB_CV.py
# GA hyperparameter optimization removed (fixed params used instead).
# Adds run_learning_curve() method to EnsembleClassifier for learning curve analysis.
import sys
import threading
...
class RandomForestModel:
    ...
    def _optimize_hyperparameters(self, X_train, y_train, class_name, fold_idx):
        """Return fixed hyperparameters (GA removed)."""
        ...
        return {
            ...
            'min_samples_leaf': 3,
            'max_features': 0.5,
            'class_weight': {0: 1.0, 1: base_class_ratio},
        }
...
class EnsembleClassifier:
    ...
    def run_learning_curve(self, rf_data, xgb_data, train_fracs=None, n_reps=5):
        """
        Generate learning curves: train vs cross-validation AUC and F1
        across increasing training set sizes (OvR, same pipeline as run()).
        """
        import pandas as pd
        from sklearn.model_selection import StratifiedShuffleSplit
        from sklearn.metrics import f1_score, roc_curve, auc as sk_auc
        import matplotlib.pyplot as plt

        if train_fracs is None:
            train_fracs = [0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90]

        rf_features_all, rf_labels_all, _ = rf_data
        xgb_features_all, xgb_labels_all, _ = xgb_data

        records = []

        for frac in train_fracs:
            print(f"  Learning curve: frac={frac:.0%}", end='', flush=True)
            sss = StratifiedShuffleSplit(n_splits=n_reps, train_size=frac, random_state=42)

            rep_tr_auc, rep_te_auc = [], []
            rep_tr_f1,  rep_te_f1  = [], []
            n_tr_list = []

            for tr_idx, te_idx in sss.split(rf_features_all, rf_labels_all):
                n_tr_list.append(len(tr_idx))

                rf_data_sub  = (rf_features_all[tr_idx],  rf_labels_all[tr_idx],  np.arange(len(tr_idx)))
                xgb_data_sub = (xgb_features_all[tr_idx], xgb_labels_all[tr_idx], np.arange(len(tr_idx)))
                rf_data_te   = (rf_features_all[te_idx],  rf_labels_all[te_idx],  np.arange(len(te_idx)))
                xgb_data_te  = (xgb_features_all[te_idx], xgb_labels_all[te_idx], np.arange(len(te_idx)))

                cls_tr_auc, cls_te_auc = [], []
                cls_tr_f1,  cls_te_f1  = [], []

                for cls in self.all_classes:
                    try:
                        # Augment training subset (same as apply_data_augmentation)
                        rf_bal, xgb_bal = self.apply_data_augmentation(rf_data_sub, xgb_data_sub, cls)
                        rf_X_tr, rf_y_tr, rf_idx_tr   = rf_bal
                        xgb_X_tr, xgb_y_tr, xgb_idx_tr = xgb_bal

                        # Test: original samples only
                        rf_X_te  = rf_data_te[0]
                        xgb_X_te = xgb_data_te[0]
                        rf_y_te_bin  = (rf_data_te[1]  == cls).astype(int)

                        if rf_y_tr.sum() < 1 or (1 - rf_y_tr).sum() < 1:
                            continue

                        # Optimize & train (same as process_fold, no GA)
                        fold_idx_dummy = 0
                        rf_best  = self.rf_component._optimize_hyperparameters(rf_X_tr,  rf_y_tr,  cls, fold_idx_dummy)
                        xgb_best = self.xgb_component._optimize_hyperparameters(xgb_X_tr, xgb_y_tr, cls, fold_idx_dummy)

                        rf_model  = self.rf_component._create_model(rf_best)
                        xgb_model = self.xgb_component._create_model(xgb_best)
                        rf_model.fit(rf_X_tr, rf_y_tr)
                        xgb_model.fit(xgb_X_tr, xgb_y_tr)

                        # Test probabilities → Bayesian weight opt → threshold
                        rf_te_p  = rf_model.predict_proba(rf_X_te)[:, 1]
                        xgb_te_p = xgb_model.predict_proba(xgb_X_te)[:, 1]
                        best_w_te = self._optimize_ensemble_weight(rf_te_p, xgb_te_p, rf_y_te_bin)
                        ens_te = (1 - best_w_te) * rf_te_p + best_w_te * xgb_te_p

                        best_f1_te, best_thr_te = 0, 0.4
                        for thr in np.arange(0.1, 0.7, 0.025):
                            pred = (ens_te >= thr).astype(int)
                            f1 = f1_score(rf_y_te_bin, pred, average='weighted', zero_division=0)
                            if f1 > best_f1_te:
                                best_f1_te, best_thr_te = f1, thr

                        try:
                            fpr, tpr, _ = roc_curve(rf_y_te_bin, ens_te)
                            te_auc = sk_auc(fpr, tpr)
                        except Exception:
                            te_auc = 0.5
                        te_f1 = f1_score(rf_y_te_bin, (ens_te >= best_thr_te).astype(int),
                                         average='weighted', zero_division=0)

                        # Train probabilities (on original non-augmented train subset)
                        rf_X_tr_orig  = rf_features_all[tr_idx]
                        xgb_X_tr_orig = xgb_features_all[tr_idx]
                        rf_y_tr_bin_orig = (rf_labels_all[tr_idx] == cls).astype(int)

                        rf_tr_p  = rf_model.predict_proba(rf_X_tr_orig)[:, 1]
                        xgb_tr_p = xgb_model.predict_proba(xgb_X_tr_orig)[:, 1]
                        best_w_tr = self._optimize_ensemble_weight(rf_tr_p, xgb_tr_p, rf_y_tr_bin_orig)
                        ens_tr = (1 - best_w_tr) * rf_tr_p + best_w_tr * xgb_tr_p

                        best_f1_tr, best_thr_tr = 0, 0.4
                        for thr in np.arange(0.1, 0.7, 0.025):
                            pred = (ens_tr >= thr).astype(int)
                            f1 = f1_score(rf_y_tr_bin_orig, pred, average='weighted', zero_division=0)
                            if f1 > best_f1_tr:
                                best_f1_tr, best_thr_tr = f1, thr

                        try:
                            fpr, tpr, _ = roc_curve(rf_y_tr_bin_orig, ens_tr)
                            tr_auc = sk_auc(fpr, tpr)
                        except Exception:
                            tr_auc = 0.5
                        tr_f1 = f1_score(rf_y_tr_bin_orig, (ens_tr >= best_thr_tr).astype(int),
                                         average='weighted', zero_division=0)

                        cls_te_auc.append(te_auc); cls_tr_auc.append(tr_auc)
                        cls_te_f1.append(te_f1);   cls_tr_f1.append(tr_f1)

                    except Exception:
                        continue

                if cls_te_auc:
                    rep_te_auc.append(float(np.mean(cls_te_auc)))
                    rep_tr_auc.append(float(np.mean(cls_tr_auc)))
                    rep_te_f1.append(float(np.mean(cls_te_f1)))
                    rep_tr_f1.append(float(np.mean(cls_tr_f1)))

            if rep_te_auc:
                n_tr = int(np.mean(n_tr_list))
                for metric, tr_vals, te_vals in [('auc', rep_tr_auc, rep_te_auc),
                                                  ('f1',  rep_tr_f1,  rep_te_f1)]:
                    for split, vals in [('train', tr_vals), ('test', te_vals)]:
                        records.append({'frac': frac, 'n_train': n_tr,
                                        'metric': metric, 'split': split,
                                        'mean': float(np.mean(vals)),
                                        'std':  float(np.std(vals))})
                print(f"  AUC(cv)={np.mean(rep_te_auc):.3f}  F1(cv)={np.mean(rep_te_f1):.3f}", flush=True)
            else:
        ...
        metric_cfg = [('auc', 'AUC (macro OvR)', [0.4, 1.02]),
                      ('f1',  'F1 score (weighted)', [0.4, 1.02])]
        colors  = {'train': '#E84B3A', 'test': '#3A7EE8'}
        labels_ = {'train': 'Training', 'test': 'Cross-validation'}

        for ax, (metric, ylabel, ylim) in zip(axes, metric_cfg):
            for split in ('train', 'test'):
                sub = lc_df[(lc_df['metric'] == metric) & (lc_df['split'] == split)].sort_values('n_train')
                if sub.empty:
                    continue
                xs = sub['n_train'].values
                ym = sub['mean'].values
                ys = sub['std'].values
                ax.plot(xs, ym, 'o-', color=colors[split], lw=2.2,
                        label=labels_[split], markersize=7, zorder=3)
                ax.fill_between(xs, ym - ys, ym + ys, alpha=0.15, color=colors[split], zorder=2)
        ...
        plt.suptitle('Learning Curve — Ensemble (RF + XGB, OvR, fixed hyperparameters)',
                     fontsize=18, y=1.01)
        plt.tight_layout()

        for ext in ('svg', 'pdf'):
            path = os.path.join(self.config['output_path'], f'Learning_Curve_Ensemble.{ext}')
            plt.savefig(path, format=ext, bbox_inches='tight')
            print(f"  Saved → {path}")
        plt.close()

        lc_path = os.path.join(self.config['output_path'], 'Learning_Curve_raw_results.xlsx')
    ...
    def run(self):
        ...
        if all_results:
            ...
            # Merge and save evolution history logs (skipped: GA removed)
            if self.logger:
...
def main():
    ...
    try:
        ...
        # Load data once (shared between CV and learning curve)
        rf_data, xgb_data = ensemble.load_data()

        # Run standard CV evaluation
        ...
        print("=" * 80)
        ensemble.run_learning_curve(rf_data, xgb_data)
